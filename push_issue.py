...
def get_repos_id(userDict, currentSession: requests.sessions): 
    ...
    print(f"https://api.github.com/repos/{userDict['owner']}/{userDict['repos_url'].split("/")[-1]}")
    ...
    if json_data and json_data.get("node_id"): 
        print(f"Found repos id for {userDict.get('repos_url')} == {json_data.get('node_id')}")

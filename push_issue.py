...
def connect_to_mongodb(url):
    try:
        # Parse and encode the username and password in the connection string
        if "@" in url:
            prefix, suffix = url.split("@", 1)
            if ":" in prefix:
                cluster, user_pass = prefix.split("//", 1)
                username, password = user_pass.split(":", 1)
                encoded_user_pass = f"{quote_plus(username)}:{quote_plus(password)}"
                url = f"mongodb+srv://{encoded_user_pass}@{suffix}"
...
def CheckIfIssueExists(repos_id, ownDBClient: pymongo.MongoClient):
    if ownDBClient:
        ...
        logging.error("No connection to MongoDB")
